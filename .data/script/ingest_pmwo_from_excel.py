#!/usr/bin/env python3
"""
Ingestion de PMW0/PMWO.xlsx -> PostgreSQL (DB: pegasus, table: public.pmwo)

Caractéristiques:
- Normalisation des colonnes (snake_case ASCII) + mapping surchargeable (--colmap).
- Clés: PK = pmwo (dérivée de '# PMWO' / 'PMWO' / 'PMW0'), FK = hts_sn_id (depuis 'Site')
        -> REFERENCES public.sites(hts_site_id) ON UPDATE CASCADE ON DELETE RESTRICT.
- Création/complétion de la table public.pmwo selon les colonnes réelles du fichier.
- Conversions robustes (bool, int/float avec virgule, timestamp, timedelta). Tous les NaN/NaT -> NULL.
- Upsert idempotent par lots (ON CONFLICT (pmwo) DO UPDATE).
...
DEFAULT_COLMAP: Dict[str, str] = {
    ...
    "PMW0": "pmwo",        # zéro -> pmwo
    "PMW0 ID": "pmwo",
    "Site": "hts_sn_id",   # on homogénéise la FK avec TRB/Sites
    "SITE": "hts_sn_id",
}
...
def ensure_schema(cur, schema: str, table: str, df: pd.DataFrame) -> None:
    """
    Crée ou complète public.pmwo en fonction des colonnes du fichier:
    - PK sur pmwo (text NOT NULL)
    - FK hts_sn_id -> public.sites(hts_site_id)
    - Index sur hts_sn_id
    """
    cols = list(df.columns)
    if "pmwo" not in cols or "hts_sn_id" not in cols:
        raise SystemExit("Colonnes obligatoires manquantes après normalisation: 'pmwo' (PK) et 'hts_sn_id' (FK).")

    if not table_exists(cur, schema, table):
        col_ddls: List[str] = []
        for c in cols:
            if c == "pmwo":
                col_ddls.append("pmwo text NOT NULL")
            elif c == "hts_sn_id":
        ...
        CREATE TABLE {quote_ident(schema)}.{quote_ident(table)} (
            {', '.join(col_ddls)},
            CONSTRAINT {quote_ident(table + '_pkey')} PRIMARY KEY (pmwo)
        );
        """
        cur.execute(ddl)
        ...
        if not pk_exists(cur, schema, table):
            cur.execute(f"""
                ALTER TABLE {quote_ident(schema)}.{quote_ident(table)}
                ADD CONSTRAINT {quote_ident(table + '_pkey')} PRIMARY KEY (pmwo);
            """)
        fk_name = table + "_hts_sn_id_fkey"
...
def prepare_dataframe(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    - Force 'pmwo' et 'hts_sn_id' en texte nettoyé
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument("excel_path", help="Chemin du fichier Excel (ex: PMW0.xlsx)")
    p.add_argument("--schema", default="public")
    p.add_argument("--table", default="pmwo")
    p.add_argument("--sheet", default="PMW0", help="Nom ou index de l'onglet (par défaut 'PMW0')")
    ...
    # Exigences minimales
    if "pmwo" not in df.columns and "# pmwo" in df.columns:
        df = df.rename(columns={"# pmwo": "pmwo"})
    if "pmwo" not in df.columns:
        raise SystemExit("Impossible de trouver la clé primaire: fournissez une colonne '# PMWO'/'PMWO'/'PMW0' (normalisée en 'pmwo').")
    ...
    print(f"Ingestion PMWO terminée: {len(df)} ligne(s) upsert dans {args.schema}.{args.table}")
    return 0
